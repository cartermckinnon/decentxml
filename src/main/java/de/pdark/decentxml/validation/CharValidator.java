...
public class CharValidator {
  ...
  public String isValid(XMLSource source, int offset) {
    ...
    if (Character.isHighSurrogate(c)) {
      if (offset + 1 >= source.length()) return "Unexpected end of input";

      char c2 = source.charAt(offset + 1);
      if (Character.isLowSurrogate(c2)) return isValid(Character.toCodePoint(c, c2));

      return "Character after first in surrogate pair is not between 0xDC00 and 0xDFFF: "
  ...
  public String isValid(int codePoint) {
    if (codePoint == '\t'
        ...
        || (0x10000 <= codePoint && codePoint <= 0x10ffff)) return null;

    return "Allowed values are #x09 | #x0a | #x0d | [#x0020-#xD7FF] | [#xE000-#xFFFD] |"
               + " [#x10000-#x10FFFF]: [#x"
  ...
  public boolean isIdeographic(char c) {
    return (0x4E00 <= c && c <= 0x9FA5) || c == 0x3007 || (0x3021 <= c && c <= 0x3029);
  }
